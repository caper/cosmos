    ...
    public function __call($m, $a) {
        try {
            ...
            if(!is_object($response)) {
                ...
                self::$history[] = new Prodom_Api_History(array('elapsed' => $t, 'method' => "{$this->serviceName}.{$m}", 'code' => $error_code, 'result' => $error_text));
            ...
            if(isset($response->error)) {
                ...
                // сохраняем ответ в историю запросов
                self::$history[] = new Prodom_Api_History(array('elapsed' => $t, 'method' => "{$this->serviceName}.{$m}", 'code' => $error_code, 'result' => $response->error->message));
                // генерим исключение
                throw new Exception($response->error->message, $error_code);
            }
            // сохраняем ответ в историю запросов
            self::$history[] = new Prodom_Api_History(array('elapsed' => $t, 'method' => "{$this->serviceName}.{$m}", 'code' => 0, 'result' => null));
